...
5. If you ever notice that GitHub Pages keeps serving an older cached version of the CSS/JS, bump the `ASSET_VERSION` constant in `script.js` and update the `?v=` query strings in `index.html`. That forces browsers/CDNs to fetch the latest assets. The page also sends `no-cache` hints via meta tags, but version bumps guarantee a refresh when caches misbehave.
